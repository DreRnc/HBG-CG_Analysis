...
class GridSearch:
    def __init__(self, optimizer, model, objective="validation_error"):
        ...
        if objective not in ["validation_error", "training_objective"]:
            raise ValueError(
                'objective must be either "validation_error" or "training_objective"'
            )
    ...
    def create_folds(self, n_folds, stratified):
        ...
        if stratified and self.objective == "training_objective":
            raise Warning(
                'stratified is non influent when the objective is "training_objective"'
            )
        if self.objective == "training_objective" and n_folds != 1:
            raise Warning(
                'n_folds is non influent when the objective is "training_objective"'
            )
    ...
    def grid_search(self, par_combinations):
        ...
        if self.parallel:
            ...
            results = Parallel(n_jobs=num_cores)(
                delayed(self.fit_model)(par) for par in tqdm(par_combinations)
            )
        else:
        ...
        if self.objective == "validation":
            if self.n_folds < 2:
                ...
                for train_index, val_index in self.folds:
                    self.fit_model_fold(
                        self.X[train_index], self.y[train_index], **parameters
                    )
                    score = self.model.evaluate_model(
                        self.X[val_index], self.y[val_index]
                    )
                    scores.append(score)
